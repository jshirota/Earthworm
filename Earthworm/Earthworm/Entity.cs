...
public class Entity : IEntity, INotifyPropertyChanged
{
    ...
    private readonly Dictionary<string, object?> _temporaryStorage = new();
    ...
    /// The ObjectID field.
    /// </summary>
    public int OID { get { return Row?.HasOID == true ? Row.OID : -1; } }

    /// <summary>
    /// Indicates if the object is bound to an actual row in the underlying table.
    /// </summary>
    public bool IsDataBound { get { return Row is not null; } }
    ...
    internal void CopyTo(IRowBuffer rowBuffer)
    {
        ...
        if (HasShape && row is IFeature feature && this is IEntity<IGeometry> entity)
...
public class Entity<T> : Entity, IEntity<T> where T : class, IGeometry
{
    ...
    public T? Shape
    {
        get { return (IsDataBound ? (Row as IFeature)?.Shape : TemporaryShape) as T; }
        set
        {
            if (Row is null)
            {
            ...
            {
                if (Row is not IFeature feature)
