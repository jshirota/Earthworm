...
public static class TableWriter
{
    ...
    private static IField CreateField(Type propertyType, string fieldName, int? textLength)
    {
        ...
        throw new ArgumentException(string.Format("This property type '{0}' is not supported.", propertyType.Name), nameof(propertyType));
    }
    ...
    private static ITable CreateTable(object container, string name, string oidField, bool isSpatial, esriGeometryType geometryType, ISpatialReference spatialReference, List<IField> customFields)
    {
        ...
        {
            IFeatureWorkspace featureWorkspace => isSpatial
                ? (ITable)featureWorkspace.CreateFeatureClass(name, fields, ocDesc.InstanceCLSID, ocDesc.ClassExtensionCLSID, esriFeatureType.esriFTSimple, "Shape", "")
                : featureWorkspace.CreateTable(name, fields, ocDesc.InstanceCLSID, ocDesc.ClassExtensionCLSID, ""),

            IFeatureDataset featureDataset =>
                (ITable)featureDataset.CreateFeatureClass(name, fields, ocDesc.InstanceCLSID, ocDesc.ClassExtensionCLSID, esriFeatureType.esriFTSimple, "Shape", ""),
    ...
    {
        return (IFeatureClass)CreateTable<T>(featureWorkspace, name, true, geometryType, spatialReference);
    }
    ...
    {
        return (IFeatureClass)CreateTable<T>(featureWorkspace, name, true, geometryType, Shape.WKID(wkid));
    }
    ...
    {
        return (IFeatureClass)CreateTable<T>(featureDataset, name, true, geometryType, ((IGeoDataset)featureDataset).SpatialReference);
    }
}
